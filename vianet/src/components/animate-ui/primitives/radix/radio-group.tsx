...
function RadioGroupIndicator({
  ...
  return (
    <AnimatePresence>
      {isChecked && (
        <RadioGroupPrimitive.Indicator data-slot="radio-group-indicator" forceMount render={<motion.div key="radio-group-indicator-circle" data-slot="radio-group-indicator-circle" initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} transition={transition} {...props} />}></RadioGroupPrimitive.Indicator>
      )}
    </AnimatePresence>
  );
}
...
function RadioGroupItem({
  ...
  return (
    <RadioGroupItemProvider value={{ isChecked, setIsChecked }}>
      <RadioGroupPrimitive.Item value={valueProps} disabled={disabled} required={required} render={<motion.button data-slot="radio-group-item" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} {...props} />}></RadioGroupPrimitive.Item>
