...
function Cursor({ className, ...props }: CursorProps) {
  return (
    <CursorPrimitive {...props} render={<svg className={cn('size-6 text-foreground', className)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40" />}><path
                fill="currentColor"
...
function CursorFollow({
  ...
  return (
    <CursorFollowPrimitive sideOffset={sideOffset} alignOffset={alignOffset} {...props} render={<div className={cn(
                'bg-foreground rounded-md text-background px-2 py-1 text-sm',
                className,
              )} />}>{children}</CursorFollowPrimitive>
  );
}
