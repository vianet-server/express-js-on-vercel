...
async function listProductsV1({ accessGroupId, search, page, limit }) {
    ...
    const countResult = await neonDb.query(`SELECT COUNT(*)::int AS total FROM app.stock s ${text.replace(selectCols, '')}`, params);
    const total = countResult.rows[0]?.total || 0;
    const dataResult = await neonDb.query(`${text} ${baseWhere} ORDER BY s.stockname LIMIT $${idx} OFFSET $${idx + 1}`, [...params, ...(search ? [`%${search}%`] : []), limit, offset]);
...
async function createProductV1({ name, quantity, price, accessGroupId }) {
    ...
    if (accessGroupId) {
        await neonDb.query('INSERT INTO app.inventory_access_group (inventoryid, accessgroupid, quantity, oprice) VALUES ($1, $2, $3, $4)', [product.id, accessGroupId, quantity || 0, price || 0]);
    }
    return product;
}
/**
...
async function updateProductV1({ id, name, quantity, price, accessGroupId }) {
    ...
    if (product && accessGroupId && (quantity != null || price != null)) {
        await neonDb.query('UPDATE app.inventory_access_group SET quantity = COALESCE($1, quantity), oprice = COALESCE($2, oprice) WHERE inventoryid = $3 AND accessgroupid = $4', [quantity ?? null, price ?? null, id, accessGroupId]);
    }
    return product;
}
/**
...
async function logApiUsage({ keyid, endpoint, method, status }) {
    await neonDb.query('INSERT INTO api_key_log (api_key_id, endpoint, method, status) VALUES ($1, $2, $3, $4)', [keyid, endpoint, method, status]);
}
module.exports = {
    ...shared,
