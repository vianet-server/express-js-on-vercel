...
async function createUser({ name, email, password_hash, user_type, access_group_id }) {
    const result = await neonDb.query('INSERT INTO app.users (name, email, password, user_type, access_group_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, NOW(), NOW()) RETURNING id, email, user_type', [name || email, email, password_hash, user_type, access_group_id]);
    return result.rows[0];
}
/**
...
async function createUserNoName({ email, password_hash, user_type, access_group_id }) {
    const result = await neonDb.query('INSERT INTO app.users (email, password, user_type, access_group_id, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING id, email, user_type', [email, password_hash, user_type, access_group_id]);
    return result.rows[0];
}
/**
...
async function createPartnerProfile({ user_id, company_name, phone, address }) {
    await neonDb.query('INSERT INTO partner_profiles (user_id, company_name, phone, address, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())', [user_id, company_name, phone, address]);
}
/**
...
async function updatePartnerProfileByUserId({ user_id, company_name, phone, address }) {
    await neonDb.query('UPDATE partner_profiles SET company_name = $1, phone = $2, address = $3, updated_at = NOW() WHERE user_id = $4', [company_name, phone, address, user_id]);
}
/**
...
async function createStockItemGuid({ name, guid, quantity, price }) {
    const result = await neonDb.query('INSERT INTO app.stock (stockname, guid, quantity, price, masterid, created_at, updated_at) VALUES ($1, $2, $3, $4, 0, NOW(), NOW()) RETURNING *', [name, guid, quantity, price]);
    return result.rows[0];
}
/**
...
async function updateStockItemById({ id, name, quantity, price }) {
    const result = await neonDb.query('UPDATE app.stock SET stockname = $1, quantity = $2, price = $3, updated_at = NOW() WHERE id = $4 RETURNING *', [name, quantity, price, id]);
    return result.rows[0];
}
/**
...
async function createLedger({ guid, name, address, mobile }) {
    const result = await neonDb.query('INSERT INTO app.ledger (guid, name, address, mobile) VALUES ($1, $2, $3, $4) RETURNING *', [guid, name, address ? [address] : null, mobile ? [mobile] : null]);
    return result.rows[0];
}
/**
...
async function updateLedger({ id, name, address, mobile }) {
    const result = await neonDb.query('UPDATE app.ledger SET name = $1, address = $2, mobile = $3 WHERE id = $4 RETURNING *', [name, address ? [address] : null, mobile ? [mobile] : null, id]);
    return result.rows[0];
}
/**
...
async function createVoucher({ guid, date, voucher_type, voucher_number, party_ledger_name, narration }) {
    const result = await neonDb.query(`INSERT INTO app.vouchers (guid, date, voucher_type, voucher_number, party_ledger_name, narration)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [guid, date, voucher_type, voucher_number, party_ledger_name, narration]);
    return result.rows[0];
}
/**
...
async function listVouchers({ voucher_type, voucher_number, from_date, to_date } = {}) {
    ...
    if (voucher_number) {
        query += ` AND voucher_number = $${idx++}`;
        params.push(voucher_number);
    }
    if (from_date) {
        query += ` AND date >= $${idx++}`;
...
async function updateVoucher({ id, voucher_type, voucher_number, date, narration, party_ledger_name }) {
    const result = await neonDb.query(`UPDATE app.vouchers SET voucher_type = $1, voucher_number = $2, date = $3,
     narration = $4, party_ledger_name = $5 WHERE id = $6 RETURNING *`, [voucher_type, voucher_number, date, narration, party_ledger_name, id]);
    return result.rows[0];
}
/**
...
async function createGodown({ name, address }) {
    const result = await neonDb.query('INSERT INTO godowns (name, address, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING *', [name, address]);
    return result.rows[0];
}
/**
...
async function updateGodown({ id, name, address }) {
    const result = await neonDb.query('UPDATE godowns SET name = $1, address = $2, updated_at = NOW() WHERE id = $3 RETURNING *', [name, address, id]);
    return result.rows[0];
}
/**
...
async function createInventoryRecord({ stockitem_id, godown_id, quantity }) {
    const result = await neonDb.query('INSERT INTO inventory (stockitem_id, godown_id, quantity, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW()) RETURNING *', [stockitem_id, godown_id, quantity]);
    return result.rows[0];
}
/**
...
async function listInventoryRecords({ stockitem_id, godown_id } = {}) {
    ...
    let idx = 1;
    if (stockitem_id) {
        query += ` AND stockitem_id = $${idx++}`;
        params.push(stockitem_id);
    }
    if (godown_id) {
        query += ` AND godown_id = $${idx++}`;
...
async function updateInventoryRecord({ id, stockitem_id, godown_id, quantity }) {
    const result = await neonDb.query('UPDATE inventory SET stockitem_id = $1, godown_id = $2, quantity = $3, updated_at = NOW() WHERE id = $4 RETURNING *', [stockitem_id, godown_id, quantity, id]);
    return result.rows[0];
}
/**
