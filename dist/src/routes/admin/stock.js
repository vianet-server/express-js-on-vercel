...
router.get('/masters', async (req, res) => {
    try {
        ...
        res.json(counts.map((c) => ({ ...c, lastUpdated: '', status: 'Active' })));
    }
    catch (err) {
...
router.get('/salesman', async (req, res) => {
    try {
        ...
        res.json(result.map((r) => ({ id: r.id, name: r.name, orders: r.orders, sales: parseFloat(r.sales) || 0, region: '', status: 'Active', target: 0, achieved: 0, commission: 0 })));
    }
    catch (err) {
...
router.get('/salesman-chart', async (req, res) => {
    try {
        ...
        res.json(result.map((r) => ({ date: r.sales_date, name: r.salesman, sales: parseFloat(r.sales) || 0 })));
    }
    catch (err) {
