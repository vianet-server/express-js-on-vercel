...
router.post('/stock-item', async (req, res) => {
    try {
        ...
        const result = await neonDb.query('INSERT INTO app.stock (stockname, guid, quantity, price, masterid, created_at, updated_at) VALUES ($1, $2, $3, $4, 0, NOW(), NOW()) RETURNING *', [name, genGuid(), quantity, price]);
...
router.post('/ledger', async (req, res) => {
    try {
        ...
        const result = await neonDb.query('INSERT INTO app.ledger (guid, name, address, mobile) VALUES ($1, $2, $3, $4) RETURNING *', [genGuid(), name, address ? [address] : null, mobile ? [mobile] : null]);
...
router.post('/voucher', async (req, res) => {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [genGuid(), date, voucher_type, voucher_number, party_ledger_name, narration]);
    ...
    try {
        ...
        const rows = result.rows.map((r) => ({
            ...
            amount: r.ledgerentries
                ? r.ledgerentries
                    .filter((e) => e.isDeemedPositive === 'No')
...
router.get('/salesman', async (req, res) => {
    try {
      ...
      ORDER BY COUNT(*) DESC
    `);
        res.json(result.rows.map((r) => ({ id: r.id, name: r.name, orders: r.orders, sales: parseFloat(r.sales) || 0, region: '', status: 'Active', target: 0, achieved: 0, commission: 0 })));
    }
    catch (err) {
...
router.get('/salesman-chart', async (req, res) => {
    try {
      ...
      ORDER BY sales_date
    `);
        res.json(result.rows.map((r) => ({ date: r.sales_date, name: r.salesman, sales: parseFloat(r.sales) || 0 })));
    }
    catch (err) {
