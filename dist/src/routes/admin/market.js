...
router.get('/', async (req, res) => {
    try {
        const now30 = await neonDb.query("SELECT COALESCE(SUM(bill_amt),0) AS total, COUNT(*) AS orders FROM app.sales_records WHERE sales_date >= CURRENT_DATE - 30");
        const prev30 = await neonDb.query("SELECT COALESCE(SUM(bill_amt),0) AS total FROM app.sales_records WHERE sales_date >= CURRENT_DATE - 60 AND sales_date < CURRENT_DATE - 30");
        const todaySales = await neonDb.query("SELECT COALESCE(SUM(bill_amt),0) AS vol FROM app.sales_records WHERE sales_date = CURRENT_DATE");
        const yesterdaySales = await neonDb.query("SELECT COALESCE(SUM(bill_amt),0) AS vol FROM app.sales_records WHERE sales_date = CURRENT_DATE - 1");
        const topProducts = await neonDb.query("SELECT stockname, quantity, price, (COALESCE(quantity,0) * COALESCE(price,0)) AS total_value FROM app.stock ORDER BY total_value DESC LIMIT 10");
        const stockCount = await neonDb.query("SELECT COUNT(*) AS cnt FROM app.stock");
        const regionData = await neonDb.query("SELECT COALESCE(NULLIF(parent, ''), 'Other') AS region, SUM(bill_amt) AS sales FROM app.sales_records WHERE sales_date >= CURRENT_DATE - 30 GROUP BY region ORDER BY sales DESC LIMIT 5");
...
router.get('/sales-trend', async (req, res) => {
    try {
        const r = await neonDb.query("SELECT sales_date::text AS day, SUM(bill_amt) AS sales, COUNT(*) AS orders FROM app.sales_records WHERE sales_date >= CURRENT_DATE - 30 GROUP BY sales_date ORDER BY sales_date");
        res.json(r.rows.map(function (x) {
            return { day: x.day, sales: parseFloat(x.sales) || 0, orders: parseInt(x.orders) || 0 };
        }));
    }
    catch (e) {
        console.error('[market] sales-trend error:', e);
        res.json([]);
    }
});
router.get('/category-data', async (req, res) => {
    try {
        const r = await neonDb.query("SELECT stockname, quantity, price, (COALESCE(quantity,0) * COALESCE(price,0)) AS total FROM app.stock ORDER BY total DESC LIMIT 10");
        ...
        console.error('[market] category-data error:', e);
        res.json([]);
    }
});
router.get('/candlestick', async (req, res) => {
    try {
        const r = await neonDb.query("SELECT sales_date, SUM(bill_amt) AS sales FROM app.sales_records WHERE sales_date >= CURRENT_DATE - 84 GROUP BY sales_date ORDER BY sales_date");
        ...
        console.error('[market] candlestick error:', e);
        res.json([]);
    }
});
