...
router.post('/api', async (req, res) => {
    try {
        ...
        const groupResult = await neonDb.query('SELECT id FROM app.access_groups WHERE name = $1', [groupName]);
...
router.delete('/access-group/:id', async (req, res) => {
    try {
        ...
        try {
            await neonDb.query('DELETE FROM app.inventory_access_group WHERE accessgroupid = $1', [numericId]);
        }
        catch (e) {
            console.warn('[api] cleanup inventory_access_group:', e.message);
        }
        try {
            await neonDb.query('UPDATE app.api SET access_group_id = NULL WHERE access_group_id = $1', [numericId]);
        }
        catch (e) {
            console.warn('[api] cleanup api:', e.message);
        }
        try {
            await neonDb.query('UPDATE app.users SET access_group_id = NULL WHERE access_group_id = $1', [numericId]);
        }
        catch (e) {
