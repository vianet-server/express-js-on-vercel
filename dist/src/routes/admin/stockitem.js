...
router.get('/inventory/sku/:sku/access-group/:group', async (req, res) => {
    try {
        ...
        if (groupRow.rows.length > 0) {
            ...
            const ia = await neonDb.query('SELECT quantity, oprice FROM app.inventory_access_group WHERE inventoryid = $1 AND accessgroupid = $2', [r.id, ag.id]);
            if (ia.rows.length > 0) {
                accessGroupData = {
                    group,
...
router.post('/inventory/sku/:sku/access-group/:group', async (req, res) => {
    try {
        ...
        if (existing.rows.length > 0) {
            await neonDb.query('UPDATE app.inventory_access_group SET quantity = $1, oprice = $2 WHERE inventoryid = $3 AND accessgroupid = $4', [qty ?? 0, price ?? 0, stockId, groupId]);
            res.json({ message: 'Stock access updated' });
        }
        else {
            await neonDb.query('INSERT INTO app.inventory_access_group (inventoryid, accessgroupid, quantity, oprice) VALUES ($1, $2, $3, $4)', [stockId, groupId, qty ?? 0, price ?? 0]);
...
router.put('/inventory/sku/:sku/access-group/:group', async (req, res) => {
    try {
        ...
        if (groupRow.rows.length === 0)
            return res.status(404).json({ message: 'Access group not found' });
        const groupId = groupRow.rows[0].id;
        const result = await neonDb.query('UPDATE app.inventory_access_group SET quantity = $1, oprice = $2 WHERE inventoryid = $3 AND accessgroupid = $4 RETURNING *', [qty ?? 0, price ?? 0, stockId, groupId]);
        if (result.rows.length === 0)
