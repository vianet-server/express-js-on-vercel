...
router.get('/', async (req, res) => {
    try {
        const result = await neonDb.query('SELECT userid, email, usertype, is_active, created_at, updated_at FROM app.users WHERE usertype = $1', ['employee']);
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await neonDb.query('INSERT INTO app.users (email, password_hash, usertype, is_active, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING userid, email, usertype, is_active', [email, password_hash, 'employee', is_active ?? true]);
        if (employee_id || first_name || last_name || phone || designation) {
            await neonDb.query('INSERT INTO employee_profiles (user_id, employee_id, first_name, last_name, phone, designation, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())', [result.rows[0].userid, employee_id, first_name, last_name, phone, designation]);
        }
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await neonDb.query('SELECT userid, email, usertype, is_active, created_at, updated_at FROM app.users WHERE userid = $1 AND usertype = $2', [id, 'employee']);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const userResult = await neonDb.query('UPDATE app.users SET email = $1, is_active = $2, updated_at = NOW() WHERE userid = $3 AND usertype = $4 RETURNING userid, email, usertype, is_active', [email, is_active, id, 'employee']);
