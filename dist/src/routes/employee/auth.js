...
router.post('/register', async (req, res) => {
    try {
        ...
        const result = await neonDb.query('INSERT INTO app.users (email, password_hash, usertype, is_active, created_at, updated_at) VALUES ($1, $2, $3, true, NOW(), NOW()) RETURNING userid, email, usertype', [email, password_hash, 'employee']);
        ...
        const token = jwt.sign({ id: result.rows[0].userid, email: result.rows[0].email, usertype: 'employee' }, process.env.JWT_SECRET, { expiresIn: '24h' });
