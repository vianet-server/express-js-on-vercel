...
router.post('/register', async (req, res) => {
    try {
        ...
        const result = await neonDb.query('INSERT INTO app.users (email, password_hash, usertype, is_active, created_at, updated_at) VALUES ($1, $2, $3, true, NOW(), NOW()) RETURNING userid, email, usertype', [email, password_hash, usertype || 'user']);
...
router.post('/signup-with-token', async (req, res) => {
    try {
        ...
        const password_hash = await bcrypt.hash(password, 10);
        const result = await neonDb.query('INSERT INTO app.users (name, email, password_hash, usertype, access_group_id, is_active, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, true, NOW(), NOW()) RETURNING userid, email, usertype', [name || email, email, password_hash, 'user', access_group_id]);
        const authToken = jwt.sign({ id: result.rows[0].userid, email: result.rows[0].email, usertype: result.rows[0].usertype }, process.env.JWT_SECRET, { expiresIn: '24h' });
...
router.post('/login', async (req, res) => {
    try {
        ...
        //   return res.status(401).json({ message: 'Invalid credentials', token: null });
        // }
        const token = jwt.sign({ id: user.userid, email: user.email, usertype: user.usertype }, process.env.JWT_SECRET, { expiresIn: '24h' });
